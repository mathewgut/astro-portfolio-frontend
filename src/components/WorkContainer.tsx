...
type ActiveItemType = WorkArray["data"][number];

type ActiveItemContextType = {
    activeItem: ActiveItemType | null;
    setActiveItem: React.Dispatch<React.SetStateAction<ActiveItemType | null>>;
};

const ActiveItemContext = createContext<ActiveItemContextType>({
    activeItem: null,
    setActiveItem: () => {},
})
                            ...
                            }
                            {
                                ...
                                <article className="flex flex-col gap-2 justify-center items-center px-6">
                                    ...
                                    <div className="flex gap-2 mb-2 text-sm">
                                        <span className="flex gap-1 capitalize justify-center items-center text-white bg-black py-1 px-2 rounded-lg">
                                            <img src="/user.svg" alt="Role icon" className="h-5 w-5" style={{ filter: "invert(1)" }} />
                                            {activeItem.role}
                                        </span>
                                        <span className="flex capitalize gap-1 justify-center items-center text-white bg-black py-1 px-2 rounded-lg">
                                            <img src="/user.svg" alt="Role icon" className="h-5 w-5" style={{ filter: "invert(1)" }} />
                                            {activeItem.type}
                                        </span>
                                    </div>
...
function ProjectItem ({item}:{item:WorkArray["data"][number]}) {
    ...
    return(
        <div
        onClick={() => activeItem.setActiveItem(item)} 
